<template>
    <div class="flex justify-center md:text-3xl text-xl">
        <input @keyup.enter="searchForDronie()" v-model="searchId" type="number" placeholder="e.g. 1234" class="text-center bg-transparent border border-green-400 active:border-green-400 focus:border-green-400 outline-none text-green-400 font-bold md:p-4 p-2"/>
        <button  @click="searchForDronie()" class="bg-slate-600 hover:bg-slate-700 transition duration-100 text-white px-8 font-bold">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd" />
            </svg>
        </button>
        <div v-if="modalIsOpen">
            <dronie-detail-card v-if="error.message == ''" :dronie="dronie"></dronie-detail-card>
            <search-error :error="error" v-if="error.message != ''"></search-error>
        </div>
    </div>
</template>
...
import SearchError from './SearchError.vue'
    export default {
  components: { DronieDetailCard, SearchError },
        data() {
           return {
                dronie : {},
                searchId: '',
            ...
            searchForDronie() {
                ...
                axios.get('/api/dronies/getByNftId/' + self.searchId)
                    .then(function (response) {
                        self.dronie = response.data
                        self.showModal()
                    }).catch(function (error) {
                        self.error.message = "Oops! Couldn't find that dronie."
                        self.error.status = error.response.status
                        self.showModal()
                    })
            },
            setDronieData() {
